...
worker or reaper. Replay drivers and deterministic schedulers use the
supervised caller-driven polling boundary:

```rust
pub enum SupervisedCleanupPoll {
    Ready(Result<CleanupProgress, SupervisedCleanupError>),
    Pending,
}
...
rejection may still consume its already-reserved trace record.
...
`SchedulerTurnId`, work used, requests advanced, completions made ready and
`Idle`/`MoreRequired`. It never exposes a lane, call disposition or low-level
`Pending`. Public turns are non-reusing and deterministically ordered. When a
turn or its aggregate progress can affect application behavior, live execution
records its ID, normalized work budget, selected request IDs and aggregate
result; replay checks that record before work and reproduces it. Internal
`Pending` poll count/frequency remains non-semantic.

The bounded ready queue emits only a privately constructed, must-use, non-
cloneable `ReadyCleanup` for an actually completed request. Claim consumes that
token and returns the stored `CleanupCompletion`; safe code cannot claim the
same token twice. Both the token's read-only accessor and the completion carry
the original `CleanupRequestId`, so callers can correlate out-of-order results
without receiving a lane or driver capability. Unknown, forged, duplicated,
stale, cross-generation and already-claimed tokens fail explicitly without
disturbing another completion. A completion remains stored until claim or a
`PlanReceipt`-bounded retirement policy, including its age/turn bound, selects
a deterministic retirement turn and records its request ID and reason. Losing
or forgetting `CleanupRequestId` or `ReadyCleanup` does not free/reuse the
...
calls without a handwritten unsafe `Sync` implementation. `PlanReceipt`
privately constructs the bounded set of `#[must_use]`, non-`Copy`, non-
cloneable `CleanupLane` capabilities before execution; there is no public
constructor, serialization or deserialization path. Each lane carries a
deterministic `CleanupLaneId`, binds the same supervisor, executor namespace,
plan and trace generation, and owns a checked non-wrapping per-lane call
sequence. Lane IDs come only from the deterministic plan/scheduler; OS thread
IDs, async task identities, call arrival order and cleaner-CAS order are never
...
rejected without changing the active call. Stale, duplicate, exhausted or
cross-supervisor lane use is likewise rejected before executor mutation.

In live mode the supervisor then invokes a crate-private executor primitive.
Synchronized interior registry state contains an atomic single-cleaner guard;
a losing primitive call returns
`CleanupStartError::Busy(CleanupContentionReceipt)` before changing any
registry, payload, cleanup or admission state. The failed CAS is the
...
driver chooses when to poll again within an explicit scheduler work budget.
Preflight rejects an invalid or zero budget without mutation.
...
Every replay-relevant lane call finalizes one bounded `CleanupCallEvent`:

```rust
enum CleanupCallDisposition {
    Busy,
    Granted {
        order: CleanupOrder,
...
The event binds `(CleanupLaneId, CallSequence)`, normalized input budget,
executor and trace generations, and its disposition. A granted result includes
the exact bounded selected/retired generation-bearing `JobId` lists, work
consumed and resulting `CleanupProgress`; a digest may supplement but never
replace those exact identities. Selected entries remain non-reusable
`Cleaning` state until the reserved event is finalized. Only then may the
supervisor publish reusable/retired states, release the guard and return
progress. Incomplete reservation/grant/result records are not replayable.
Post-grant recording contradiction or corruption is process-terminal or makes
the executor/trace generation unavailable; it never returns ordinary progress
after unrecorded mutation.

`CleanupContentionReceipt` and `CleanupGrantReceipt` are bounded, opaque,
`#[must_use]`, non-cloneable and privately constructed, with no public
deserialization path. Each binds the executor namespace, plan/trace generation
and its CAS observation. The raw primitive and receipt types are crate-private
and never cross the supervised recording boundary into application code. A
losing call finalizes its reserved event as `Busy` before returning observable
`SupervisedCleanupError::Busy`. Recording rejects stale, duplicate or misbound
lanes, receipts and orders. Trace exhaustion returns `TraceUnavailable` or
invokes the existing stop/resynchronize/replay-unavailable policy; it never
leaks ordinary unrecorded `Busy` or progress.

In replay mode the supervisor looks up the exact
`(CleanupLaneId, CallSequence)` before any live cleaner CAS. `Busy` returns
without a CAS. `Granted` may run only when its recorded non-wrapping global
`CleanupOrder` is next; an early call returns
`SupervisedCleanupPoll::Pending`, and its lane retains the same active logical
call. `Pending` performs no executor, lane-sequence, trace, replay-cursor or
application-result transition and is neither an error nor a recorded
application outcome. Poll count, frequency and arrival order never enter
cleanup semantics. The ordered replay gate, not live CAS timing, owns the
cleaner. Before destructive work it checks the budget and predicted exact
selection; after work it checks selected/retired identities, work and
`CleanupProgress` against the finalized event before advancing the replay
cursor. Missing, duplicate, exhausted, incomplete or contradictory events/
orders make replay unavailable, with post-mutation mismatch following fail-
...
poll was pending. A controlled adapter may block for recorded order only in an
explicitly admitted blocking profile with bounded cancellation/liveness
...
grants/results remain recorded for every replayable concurrent profile. Even
an inert profile uses the supervisor and a planned lane. There is no public
raw-cleanup escape hatch.
...
- aliased/detached/unresponsive parallel work, forgotten/leaked handles or
  ...
  duplicate or mismatched supervisors, implicit/arrival-ordered cleanup lanes,
  OS thread/task identity as replay identity, cross-supervisor or concurrently
  reused lanes, lane-sequence wrap/exhaustion, unrecorded successful grants,
  reversed grant order, live-CAS ownership during replay, under-reserved or
  ...
  exclusive driver borrowing that serializes distinct lanes, missing/unbounded
...
51. Tier 0 remains thread-free; Tier 2 multicore execution and runtime source
    ...
    crate-private raw primitive/receipt, plan-issued non-cloneable mutable
    cleanup lanes, deterministic non-ambient lane IDs, checked non-wrapping
