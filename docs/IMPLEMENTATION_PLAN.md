...
digests cannot convert into it. Nonce and counter persistence is crash-safe;
encryption/counter commit is atomic through either binding-based compare-and-
advance or an exclusive expiring writer reservation spanning binding, seal,
durable commit and monotonic advancement; separate calls do not qualify.
Reservation expiry uses authority monotonic state and boot generation, never
UTC/wall/caller time. Concurrent writers, reboot recovery, reservation expiry,
counter exhaustion, cloned/restored authority state and every crash boundary
fail closed. Rotation/migration is explicit; outer resource validation
precedes decryption into caller buffers; and authentication failure is uniform.
...
Explicit terminal-result, consuming `join` and `cancel_and_join` APIs are the
normal completion paths; recovery occurs only after completed/cancelled/
failed-returned state. Dropping a terminal-returned handle releases returned
resources. Dropping a nonterminal handle, including during unwinding, invokes
`std::process::abort()` directly; `Drop` never joins, detaches, allocates,
formats, panics or unwinds. No quarantine/reaper or executor-ownership-transfer
profile is admitted for 1.0.
...
forgotten handles, and an unresponsive job traps shutdown. Executor drop with
registered work aborts; forgetting the executor leaks all registered capacity
until process termination. Soundness never depends on a destructor running,
and no hidden `Arc`, allocation, worker or lease bypasses planning. Lease state
is `CallerOwned -> Submitted -> WorkerOwned -> Returned`; cancellation
requests, deadline misses and unresponsive reports do not change ownership.
...
or `StateUnknown { cause, evidence }`. Private-construction no-mutation proof
binds the plan/device/prior generation and exact proof point.
Applied-transition evidence binds the new configuration and required
per-device commands/ACK/read-back, but remains only an adapter/device
transaction assertion—not independently observed rate, timing, calibration or
coherence. Timeout, disconnect, lost acknowledgement, post-submission
transport failure, uncertain rollback or required proof/evidence-capacity
exhaustion is `StateUnknown`; overflow is recorded rather than discarded.
...
failures, receiver-asserted configuration/partial-application errors, missing
SDR success/no-mutation proof, lost failure causes/evidence, unplanned/partial/
unknown transitions, stale samples or false coherence, escaped borrowed work,
forgotten/leaked handles or executors, detached registry entries, hidden owned
leases, unresponsive parallel work, premature capacity/buffer reuse, trace
...
| Snapshot envelope, narrow counter checks, domain-separated transactional binding and platform protection | v0.18.1-v0.18.2, v0.48.4, v0.54.2-v0.55.1, v0.144.3, v0.168.3 and v0.189.2-v0.189.6 |
...
| Front-end conditioning, capture mapping, proof/cause-carrying coherent transactions and adapter conformance | v0.37.2, v0.47.2-v0.50.3 and v0.170.0-v0.174.0 |
...
| Tier 2 executor registry, lost-handle accounting, explicit shutdown/fail-stop and lossless traces | v0.48.3 |
