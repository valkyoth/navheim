...
CONFORMANCE_MILESTONE_DETAILS = {
    ...
    "0.48.3": (
        "Expose the low-level supervised cleanup scheduler boundary as ExecutionSupervisor::poll_cleanup(&self, &mut CleanupLane, &Executor, budget) -> SupervisedCleanupPoll, with Ready(Result<CleanupProgress, SupervisedCleanupError>) or non-error Pending. Preserve PlanReceipt-issued deterministic linear lanes, live pre-CAS Busy-or-Granted reservation, private contention/grant receipts, checked global CleanupOrder, grant-before-mutation and exact result-before-publication. Replay reads finalized events without CAS and returns Pending until the recorded grant is next. Pending retains one active call and changes no executor, sequence, trace, replay cursor or application result. Only bounded replay drivers consume it; application APIs expose ready outcomes/errors. The base never blocks or spins.",
        "Use visibility/API, Miri, Loom, Kani/model and subprocess tests proving SupervisedCleanupError has no pending variant; live calls are Ready; repeated/permuted Pending polls make no executor/lane-sequence/trace/cursor/result transition; one active lane cannot start a second call or change bound arguments; poll frequency/arrival is semantically invisible; drivers obey work budgets and expose only recorded Busy/progress/genuine errors; base APIs never block/spin; blocking adapters require an admitted profile; externally Pending-dependent behavior is a documented non-claim unless scheduler-traced. Retain live-only atomic reservation/active-call commit, unadvanced reservation failure, read-only replay, bounded Busy/Granted capacity, authentic private receipts, non-wrapping order/exhaustion/renewal, grant/result publication order, exact empty/nonempty identities/work/progress, incomplete/duplicate/contradictory events, inverted schedules, no-live-CAS grants, selection mismatch/post-mutation fail-stop, lane binding/exhaustion/forget, raw boundary, trace failure, inert Busy only, live handles, overlapping cleaners, lowest-JobId/work bounds, lifecycle and dispatch/cancel races, no implicit cleanup, publication, ABA/namespace renewal, exactly-once storage, forgotten orphans, panic/reentrancy/unresponsive abort, exact APIs and non-durable pre-abort status.",
    ),
    "0.50.3": (
