...
CONFORMANCE_MILESTONE_DETAILS = {
    ...
    "0.48.3": (
        "Expose replayable cleanup only as ExecutionSupervisor::poll_cleanup(&self, &mut CleanupLane, &Executor, budget) with PlanReceipt-issued deterministic linear lanes. In live mode, after validation atomically reserve worst-case Busy-or-Granted call-event capacity and activate (CleanupLaneId, CallSequence) before CAS; failure leaves it unadvanced. A lost CAS yields a private contention receipt; a win assigns checked global CleanupOrder and a private grant receipt, records the grant before cleanup, and finalizes exact selected/retired JobIds, work and CleanupProgress before state/progress publication. Replay reads the finalized event without trace mutation, returns Busy without CAS or admits Granted only at the recorded next order; ReplayPending retains the active call. Preserve bounded cleanup, inert-Busy proof, generation safety, atomic-only Drop, fail-stop, safe ownership and no-unsafe-Sync.",
        "Use visibility/API, Miri, Loom, Kani/model and subprocess tests for linear lanes; live-only atomic worst-case reservation/active-call commit; reservation failure leaving the sequence unadvanced before CAS/mutation; read-only replay lookup; bounded Busy/Granted capacity; private authentic contention/grant receipts; non-wrapping global order/exhaustion/renewal; grant-before-mutation and result-before-publication; exact identities/work/progress including empty-selection Complete and MoreRequired cases; incomplete/missing/duplicate/contradictory events; inverted live/replay arrival and CAS schedules; ReplayPending retaining one call; ordered grants without live CAS; selection/result mismatch and post-mutation fail-stop. Retain lane binding/exhaustion/forget tests, raw boundary, trace failure, inert Busy only, live handles, overlapping cleaners, lowest-JobId/work bounds, lifecycle races, dispatch/cancel, no implicit cleanup, publication, ABA/namespace renewal, exactly-once storage, forgotten orphans, panic/reentrancy/unresponsive abort, exact APIs and non-durable pre-abort status.",
    ),
    "0.50.3": (
