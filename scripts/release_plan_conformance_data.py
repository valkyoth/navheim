...
CONFORMANCE_MILESTONE_DETAILS = {
    ...
    "0.48.3": (
        "Move low-level supervised polling off ExecutionSupervisor to ReplayDriver::poll_cleanup(&self, &ExecutionSupervisor, &mut CleanupLane, &Executor, budget) -> SupervisedCleanupPoll. ReplayDriver is must-use/non-Copy/non-Clone/non-serializable, has no public constructor, and is created exactly once per scheduler generation by consuming a sealed must-use/non-Copy/non-Clone/non-serializable SchedulerPermit from validated PlanReceipt with supervisor/executor/plan/trace/scheduler binding. Shared driver calls preserve distinct-lane concurrency; mutable lanes exclude same-lane concurrency. The ready-only application facade submits bounded work and emits CleanupCompletion only after Ready; its signatures expose no driver/lane/permit/poll type and completion has no Pending. Custom schedulers require a separately reviewed permit/profile. Preserve every lane, reservation, grant/result, replay, lifecycle and fail-stop guarantee.",
        "Use visibility/compile/API, Miri, Loom, Kani/model and subprocess tests proving ExecutionSupervisor has no public poll_cleanup; application facade signatures cannot name/receive ReplayDriver, CleanupLane, SchedulerPermit or SupervisedCleanupPoll; CleanupCompletion and SupervisedCleanupError have no pending variant; driver/permit constructors and deserialization are unavailable; permit consumption and clone/Copy/forgery/duplicate/stale/cross-binding fail; forgotten permit/driver cannot be replaced in-generation or expose raw cleanup; request/completion/driver-turn bounds hold; shared-driver distinct-lane calls remain concurrent and same-lane calls cannot overlap. Test ready-only completion, controlled custom-extension rejection, side-effect-free repeated/permuted Pending, active-call argument binding, semantic poll invisibility, driver budgets, no base block/spin and separately admitted blocking. Retain atomic reservation/failure, read-only replay, Busy/Granted capacity/receipts/order, publication/exact results, incomplete/inverted/mismatch cases, lane exhaustion/forget, raw boundary, trace failure, inert Busy, handles/cleaners/work/lifecycle/dispatch/publication/ABA/exactly-once/orphan/panic/reentrancy/unresponsive and exact-API tests.",
    ),
    "0.50.3": (
